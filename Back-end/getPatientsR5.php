...
// Fetch the patients from the database
$sql = "SELECT DISTINCT patient.*, appointment.date, appointment.time, appointment.tos
        FROM patient
        LEFT JOIN appointment ON patient.ssn = appointment.patient_ssn WHERE patient.vat_reg_num = $vatRegNum";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $patientSSN = $row["ssn"];
        $historySql = "SELECT date, time FROM history WHERE patient_ssn = '$patientSSN' AND date < CURDATE()";
